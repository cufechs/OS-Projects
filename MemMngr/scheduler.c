				...
				if (ReadyQueue != NULL)
				{
					...
					if (ReadyQueue->Value->generated == false)
					{
						...
						if (ArchiveHead == NULL)
						{
							...
							ArcInstance.memSize = ReadyQueue->Value->memSize;
		...
		while (1)
		{
			...
			if (ReadyQueue != NULL)
			{
				...
				if (ArchiveHead == NULL)
				{
					...
					ArcInstance.startMemIndex = alloc.start;
					ArcInstance.endMemIndex = alloc.end;
					ArcInstance.memSize = runningProcess->memSize;
...
void CleanUp(int signum)
{
	...
	while (1) //Writing Output file
	{
		...
		{
			char Word[20];
			if (Temp->ArchiveInstance.State == 0 && Temp->ArchiveInstance.memSize != 0) //allocated
			{
				sprintf(Word, "allocated");
				fprintf(pFile, "At time %d %s %d bytes for process %d from %d to %d\n", Temp->ArchiveInstance.EventTime, Word, Temp->ArchiveInstance.memSize, Temp->ArchiveInstance.ID, Temp->ArchiveInstance.startMemIndex, Temp->ArchiveInstance.endMemIndex);
			}

			else if (Temp->ArchiveInstance.State == 3) //freed
			{
				sprintf(Word, "freed");
				fprintf(pFile, "At time %d %s %d bytes for process %d from %d to %d\n", Temp->ArchiveInstance.EventTime, Word, Temp->ArchiveInstance.memSize, Temp->ArchiveInstance.ID, Temp->ArchiveInstance.startMemIndex, Temp->ArchiveInstance.endMemIndex);
			}
